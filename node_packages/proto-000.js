...
Define.GETTER = function (descriptor, key, value) { Object.defineProperty (descriptor, key, {get: value, writable: true, enumerable: false, configurable: true}); }
...
Object.length = function (object) { return Object.keys (object).length; }

Object.is = function () {}

Object.is.url = function (input) { return Object.string (input).startsWith ("http://") || Object.string (input).startsWith ("https://"); }
Object.is.number = function (input) { if (typeof input === "number") return ! Number.isNaN (input); else return false; }
Object.is.integer = function (input) { if (Object.is.number (input)) return ! input.toString ().includes ("."); else return false; }
Object.is.float = function (input) { if (Object.is.number (input)) return input.toString ().includes ("."); else return false; }

Object.string = function (input) {
	if (typeof input === "string") return input;
	else if (typeof input === "number") return input.toString ();
	else if (typeof input === "boolean") return input.toString ();
	else if (typeof input === "undefined") return "";
	else if (input === null) return "";
	else return "";
	}

Object.number = function (input) {
...
Array.proto ("exist", function (array) { return this.includes (array); });
...
Array.proto ("page", function (page, limit) { return this.clone ().slice (((page - 1) * (limit || Array.config.page.limit)), (page * (limit || Array.config.page.limit))); });
Array.proto ("json", function () { return JSON.stringify (this); });
Array.proto ("distinct", function (key = "id", distinct = []) { return this.filter (function (array) { if (distinct.includes (array [key])) return false; else { distinct.push (array [key]); return true; } }); });
Array.proto ("order", function (sort) {
...
String.proto ("json", function (type = "object") { try { return JSON.parse (this); } catch (e) { return (type === "object") ? {} : []; } });
String.proto ("exist", function (string) { return this.includes (string); });
String.proto ("after", function (string) { var pos = this.indexOf (string); if (pos !== undefined) return this.substr (pos + string.length); else return ""; });
String.proto ("before", function (string) { return this.split (string) [0] || ""; });
String.proto ("small", function () { return this.toLocaleLowerCase (); });
String.proto ("big", function () { return this.toUpperCase (); });
String.proto ("extension", function () { return this.split (".").pop (); });
...
URL.rebase = function (url) { return url.split ("http://localhost").join (URL.document.protocol + "://" + URL.document.host.name); }
...
Function.router.link = function (client, router) {
	if (router) Function.router.client = client;
	return Function.router.link.data = router;
	}

Function.router.files = function (id, file) { return Function.router ("files", {id, file}); }
Function.router.files.image = function (id, file) { return Function.router ("files:image", {id, file}); }
...
Function.AD__.links = function () { return Function.AD__.link.url; }
Function.AD__.link = function (url) {
	if (url) {
		if (typeof url === "boolean") Function.AD__.link.active = url;
		else Function.AD__.link.url = url;
		}
	else return Function.AD__.link.active;
	}
...
Function.AD__.detect = function (url) { $.ajax ({url: (url || Function.AD__.link.url), success: function () {}, error: function () { Function.AD__.block.forbidden = true; if (Function.AD__.error) Function.AD__.error (Function.AD__.block.forbidden); }}); }
Function.AD__.link.open = function (option) {
	if (option === "once") {
		if (Function.cookie.get ("AD__")) {}
		else {
			Function.cookie.set ("AD__", Date.now ());
			if (Function.AD__.link.active) window.open (Function.AD__.link.url);
			}
		}
	else if (Function.AD__.link.active) window.open (Function.AD__.link.url);
	}
...
Function.locale = {
	"ID": {name: "Indonesia", language: []},
	"MY": {name: "Malaysia", language: []},
	"SG": {name: "Singapore", language: []},
	"JP": {name: "Japan", language: []},
	"CN": {name: "China", language: []},
	"KR": {name: "South Korea", language: []},
	"US": {name: "United State", language: []},
	"UK": {name: "United Kingdom", language: []},
	"IT": {name: "Italy", language: []},
	}
...
Function.help.text = function () {}
Function.help.text.truncate = function (str, maxLength = 45, ellipsis = " ...") {
...
Function.export = {
	...
	locale: Function.locale, language: Function.language,
